...
if(key_exists('vd_hidden',$_POST)) {
	if ( $_POST['vd_hidden'] == 'Y' ) {
		...
		$instagram_app = $_POST['vd_instagram_app'];
		update_option( 'vd_instagram_app', $instagram_app );

		$instagram_secret = $_POST['vd_instagram_secret'];
		update_option( 'vd_instagram_secret', $instagram_secret );
		...
		$instagram_app = get_option('vd_instagram_app');
	    $instagram_secret = get_option('vd_instagram_secret');
...
<div class="wrap">
    ...
    <form name="vd_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
	    ...
	    <?php    echo "<h4>" . __( 'Instagram Settings', 'vd_trdom' ) . "</h4>"; ?>
        <p><?php _e("Instagram App Id: " ); ?><input type="text" name="vd_instagram_app" value="<?php echo $instagram_app; ?>" size="50"><?php _e("ex: QJd21c6OVDluWLrTlWziAEtk8" ); ?></p>
        <p><?php _e("Instagram App Secret: " ); ?><input type="text" name="vd_instagram_secret" value="<?php echo $instagram_secret; ?>" size="100"><?php _e("ex: 8Dz6DmAxE38uoojW99qSNgbae8mmZWayvSTK9y4v2n9jmyb8rP" ); ?></p>
